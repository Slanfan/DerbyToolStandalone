...
.controller('TournamentController', function($scope) {
    ...
    // const eventSource = new EventSource('/reported_matches');

    // eventSource.onmessage = function(event) {
    //     const data = JSON.parse(event.data);
    //     console.log('Reported matches emitted new data', data)
    // };

    // eventSource.onerror = function(error) {
    //     console.error('Reported matches failed:', error);
    // };
    ...
    app.socket.addEventListener('message', function (event) {
        ...
        if (data.origin === 'client') {
            ...
            switch (data.type) {
                case 'match-result':
                    console.log('...match result arrived', data.data)

                    debugger

                    // get the tournament id
                    const tournamentIndex = app.tournaments.findIndex(i => i.id === app.activeTournament.id)
                    if (tournamentIndex === undefined || tournamentIndex === -1) {
                        // handle error here
                        return
                    }

                    // get the batch index holding the match reported
                    const batchIndex = app.activeTournament.batches.findIndex(i => i.number === data.data.batch)
                    if (batchIndex === undefined || batchIndex === -1) {
                        // handle error here
                        return
                    }

                    // get the match index of the match reported
                    let matchIndex = app.activeTournament.batches[batchIndex].matches.findIndex(i => i.id === data.data.id)
                    if (matchIndex === undefined || matchIndex === -1) {
                        // handle error here
                        return
                    }
                    
                    app.tournaments[tournamentIndex].batches[batchIndex].matches[matchIndex] = data.data
                    ...
                    app.clientLog.push({
                        title: 'Match result reported by reporter client',
                        subTitle: `Table ${data.data.tableNumber}`,
                    ...
                    break
                case 'init':
                    console.log('...client initiated, lets check for active tournaments')
                    console.log('...app.activeTournament', app.activeTournament)
                    if (app.activeTournament && app.activeTournament.id) {
                        console.log('...found an active Tournament, lets pass it to the client', app.activeTournament)
                        app.socket.send(JSON.stringify({
                            type: 'select-tournament',
                            data: app.activeTournament,
                            message: 'Tournament selected',
                            origin: 'admin',
                        }));
                    }
                    break
    ...
    app.activateTournament = function(tournamentId) {
        ...
        // send pairings for the reporting client
        console.log('Tournament selected, emit data', app.tournaments[index])
        app.socket.send(JSON.stringify({
            type: 'select-tournament',
            data: app.tournaments[index],
            message: 'Tournament selected',
            origin: 'admin',
        }));
    ...
    app.matchListFiltered = function() {
        ...
        app.activeBatch.matches.forEach((match) => {
            if(app.matchListFilter == '') {
                ...
                if(match.lookUp.toLowerCase().includes(app.matchListFilter.toLowerCase()) || `t${match.tableNumber}` === app.matchListFilter) {
                    filteredList.push(match);
                }
            }
        });
    ...
    app.generatePairings = function() {
        ...
        matches.forEach((match, matchIndex) => {
            ...
            match.tableNumber = matchIndex + 1;
        ...
        app.socket.send(JSON.stringify({
            type: 'pairings-created',
            data: app.tournaments[index],
            message: 'Pairings created',
            origin: 'admin',
        }));
    ...
    app.exportTableTo_CSV = function(tableName) {
        ...
        if(tableName == 'matches') {
            ...
            let header = 'Segment,Player One,Player Two,Table'
            csv.push(header);
            app.activeBatch.matches.forEach(match => {
                let matchRow = match.match + ',' + match.player1.name + ',' + match.player2.name + ',' + match.tableNumber;
                csv.push(matchRow);
            });
        }
    ...
    app.getReporterUrlImage = () => {
        return `https://barcode.orcascan.com/?type=QR&data=${window.location.origin}/client`
    }
    app.closeReporter = () => {
        app.activeMatch = ''
    }
    app.closeManualPairing = () => {
        app.manualPairingMode = false
    }
});
